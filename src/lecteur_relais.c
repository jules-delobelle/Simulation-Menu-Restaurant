...
int main()
{
    ...
    if(!valider_format_requete(message_recu)){
        ...
        char message_erreur[100] = "Erreur : format invalide";
        write(fd_recu_confirmation, message_erreur, strlen(message_erreur)+1);
    ...
    if (fd_return == -1)
    {
       perror("Erreur: Le pipe de confirmation n'existe pas encore!\n\n");
        return 1; 
    }
    ...
    int fd_recu_confirmation = open(com_vers_lecteur.pipe_response, O_WRONLY);
    if (fd_recu_confirmation == -1){
        perror("Erreur: Le pipe de relais n'existe pas encore!\n\n");
        return 1;
    }
    write(fd_recu_confirmation, confirmation_relais, strlen(confirmation_relais)+1);
    close(fd_recu_confirmation);
